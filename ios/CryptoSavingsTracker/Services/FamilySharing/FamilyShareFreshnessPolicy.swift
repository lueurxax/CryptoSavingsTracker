...
struct FamilyShareFreshnessPolicy: Sendable {
    ...
    nonisolated static let activeThreshold: TimeInterval = 30 * 60          // 30 minutes

    /// Maximum age (seconds) for `recentlyStale` tier.
    nonisolated static let recentlyStaleThreshold: TimeInterval = 4 * 3600  // 4 hours

    /// Maximum age (seconds) for `stale` tier. Beyond this is `materiallyOutdated`.
    nonisolated static let staleThreshold: TimeInterval = 24 * 3600         // 24 hours

    // MARK: - Cooldowns and Debounce

    /// Minimum seconds between invitee refresh attempts per namespace.
    nonisolated static let refreshCooldown: TimeInterval = 30

    /// Debounce window for mutation-triggered dirty events.
    nonisolated static let mutationDebounce: TimeInterval = 2.0

    /// Debounce window for rate-drift dirty events.
    nonisolated static let rateDriftDebounce: TimeInterval = 5.0

    /// Exponential backoff schedule for repeated publish failures (seconds).
    nonisolated static let backoffSchedule: [TimeInterval] = [5, 15, 60, 300]

    /// Maximum time to wait for CloudKit import fence before suppressing publish.
    nonisolated static let importFenceTimeout: TimeInterval = 10.0

    /// Auto-dismiss interval for `refreshFailed` substate.
    nonisolated static let refreshFailedAutoDismiss: TimeInterval = 60.0

    /// Auto-dismiss interval for `checkedNoNewData` substate.
    nonisolated static let checkedNoNewDataAutoDismiss: TimeInterval = 120.0

    /// Clock-skew tolerance before emitting telemetry (seconds into the future).
    nonisolated static let clockSkewTolerance: TimeInterval = 60.0

    /// Rate cache TTL — the interval at which `FamilyShareForegroundRateRefreshDriver` ticks.
    nonisolated static let rateCacheTTL: TimeInterval = 300.0  // 5 minutes

    /// Periodic guard interval (safety net for missed rate refreshes).
    nonisolated static let periodicGuardInterval: TimeInterval = 15 * 60    // 15 minutes

    // MARK: - Clock

    private let clock: FamilyShareClock

    nonisolated init(clock: FamilyShareClock = SystemClock()) {
        self.clock = clock
    }
    ...
    nonisolated func effectiveAge(publishedAt: Date, rateSnapshotAt: Date?) -> TimeInterval {
    ...
    nonisolated func tier(forEffectiveAge age: TimeInterval) -> FamilyShareFreshnessTier {
        switch age {
    ...
    nonisolated func governingDependency(publishedAt: Date, rateSnapshotAt: Date?) -> FamilyShareFreshnessGoverningDependency {
        let publishAge = clampedAge(for: publishedAt).age
        let rateAge = rateSnapshotAt.map { clampedAge(for: $0).age } ?? publishAge
    ...
    nonisolated func evaluate(publishedAt: Date, rateSnapshotAt: Date?) -> (tier: FamilyShareFreshnessTier, governing: FamilyShareFreshnessGoverningDependency) {
    ...
    nonisolated func clampedAge(for timestamp: Date) -> (age: TimeInterval, skewDetected: Bool) {
        let now = clock.now()
    ...
    nonisolated func clockSkewDiagnostics(publishedAt: Date, rateSnapshotAt: Date?) -> ClockSkewDiagnostics {
        let now = clock.now()
    ...
    nonisolated func backoffDelay(forFailureCount count: Int) -> TimeInterval {
