#!/usr/bin/env python3
"""Block legacy runtime references to CryptoSavingsTrackerApp.sharedModelContainer."""
...
FORBIDDEN_PATTERNS: list[tuple[str, re.Pattern[str], str]] = [
    (
        "RPC001",
        re.compile(r"\bCryptoSavingsTrackerApp\.sharedModelContainer\b"),
        "Runtime code must use PersistenceController.activeContainer/activeMainContext instead of sharedModelContainer.",
    ),
]
...
def main() -> int:
    ...
    parser.add_argument(
        "--roots",
        nargs="+",
        default=["ios/CryptoSavingsTracker"],
    ...
    parser.add_argument(
        "--report-out",
        default="artifacts/navigation/runtime-persistence-controller-report.json",
    ...
    if findings:
        print("runtime persistence-controller check failed:")
        for item in findings:
            print(f"- {item.file}:{item.line} [{item.rule_id}] {item.message} :: {item.snippet}")
        print(f"report: {out_path}")
        return 1

    print("runtime persistence-controller check passed")
    print(f"report: {out_path}")
    return 0
